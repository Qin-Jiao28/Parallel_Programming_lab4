...
void MD5Hash_SIMD(string input[4], bit32 state[4][4])
{
	...
	for (int i = 0; i < n_blocks; i++)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			...
			uint32x4_t v = vmovq_n_u32(w0); // 填充通道0
            v = vsetq_lane_u32(w1, v, 1);   // 填充通道1
            v = vsetq_lane_u32(w2, v, 2);   // 填充通道2
            v = vsetq_lane_u32(w3, v, 3);   // 填充通道3
            x[j] = v;
		}
		uint32x4_t a = A;
		uint32x4_t b = B;
    ...
    vst1q_u32(resA, A); vst1q_u32(resB, B); 
    vst1q_u32(resC, C); vst1q_u32(resD, D);
